 ...
 * There are a variety of different settings that may be changed to modify
 * images taken by an RPiCamera instance, such as width, height, AWB, DRC, and
 * the resulting image's encoding type (jpg, bmp, png, gif).<br>
 * <br>
 * Although it provides additional features and support for Java specific
 * operations, RPiCamera is essentially a wrapper class for executing the
 * raspistill software from within a Java application. Thus it is essential that
 * all Raspberry Pis using the JRPiCam library have properly configured and
 * working raspistill software.<br>
 * <br>
 * Usage Example:<br>
 * <pre>
 *{@code
...
public class RPiCamera {
    ...
    HashMap<String, String[]> videoOptions = new HashMap<>();
    ProcessBuilder pb;
    Process p;
     ...
     *
     * @param saveDir A String specifying the directory for RPiCamera to save
     * images.
     * @throws FailedToRunRaspistillException
     */
...
///////////////////////////////////////////////////////////////////////////////
     ...
     * default). The image's encoding will be the same as the RPiCamera's
     * encoding setting (JPEG by default).<br>
     * <br>
     * Usage Example:<br>
     * <pre>
     *{@code
     ...
     * @return A File object representing the full path the picture was saved
     * to.
     * @throws IOException
...
//						"Desktop" + File.separator + "RPiCamera.out"));
    ...
    /**
     * Takes an image and saves it under the specified name to the RPiCamera's
     * save directory ("/home/pi/Pictures" by default). The image's encoding
     * will be the same as the RPiCamera's encoding setting (JPEG by
     * default).<br>
     * <br>
     * Usage Example:<br>
     * <pre>
    ...
    /**
     * Takes an image of the specified width and height and stores it in a
     * BufferedImage object. The resulting image is NOT saved anywhere in the
     * Pi's memory. The image's encoding will be the same as the RPiCamera's
     * encoding setting (JPEG by default).<br>
     * <br>
     * Usage Example:<br>
     * <pre>
...
//		--------------------------------------------------------------------------
    ...
    /**
     * Takes an image and stores it in a BufferedImage object. The resulting
     * image is NOT saved anywhere in the Pi's memory. The image's encoding will
     * be the same as the RPiCamera's encoding setting (JPEG by default).<br>
     * <br>
     * Usage Example:<br>
     * <pre>
     * {@code
    ...
    /**
     * Captures an image and returns the RGB values of that image. Images taken
     * in this manner are not encoded, rather rather their RGB values are stored
     * in an int array. Indexes 0 - 2 of the array make up the red, green, and
     * blue values of the upper left pixel in the image. The pixels are added to
     * the array from left to right, top to bottom.<br>
     * <br>
     * If an image's dimensions are not multiples of 16, then black padding will
     * be added onto the right and bottom sides of an image until both the width
     * and height are multiples of 16 (e.g. a 500x500 image would have a black
     * band 12 pixels thick added to the right and bottom sides to achieve
     * dimensions of 512x512, the closest multiple of 16 to 500).<br>
     * <br>
     * The boolean keepPadding parameter may be set to false to remove this
     * padding. Note that this does require extra computations to be performed,
     * so it has the potential to slightly reduce the fps a Pi is capable of.
     * However, this has not yet been officially tested for. If the padding is
     * kept, then it is the responsibility of the caller to determine what the
     * dimensions of the resulting image will be, and adjust their processing as
     * such (e.g. If the padding is kept on a 500x500 image, the array that is
     * returned will contain 786,432 elements, as opposed to the 750,000 one
     * would expect from a 500x500 image. This is because the image stored in
     * the returned array is not 500x500, but 512x512 and the caller of this
     * method must take that into account when processing the image).<br>
     * <br>
     * Usage Example:<br>
     * <pre>
     * {@code
     ...
     * </pre>
     *
     * @param width An int specifying the width of the image to take (ideally a
     * multiple of 16).
     * @param height An int specifying the height of the image to take (ideally
     * a multiple of 16).
     * @param keepPadding A boolean indicating whether or not to preserve the
     * padding on an image.
...
//				"Desktop" + File.separator + "RPiCamera.out"));
    ...
    /**
     * Take a series of timelapsed photos for the specified time frame and save
     * them under the specified filename to the RPiCamera's save directory.
     * Length of time to timelapse for may be specified with the setTimeout()
     * method.<br>
     * <br>
     * THIS METHOD WILL BLOCK THE THREAD UNTIL THE TIMELAPSE IS COMPLETE IF
     * boolean wait ARGUMENT IS SET TO true.<br>
     * <br>
     * Most recent image in the series may be appended to the file specified in
     * the setLinkLatestImage() method. If this property is set, timelapse()
     * will return a File object representing the full path to the file the
     * timelapsed photos are being linked to. Otherwise, timelapse() will return
     * null.<br>
     * <br>
     * When passing in a String for the photo's name, it is important to include
     * "%04d" in the name. This is where the frame count number will be included
     * in the file name. If the name does not contain these characters, the
     * frame count will be appended to the beginning of each file name.<br>
     * <br>
     * Usage Example:<br>
     * <pre>
     * {@code
     ...
     * </pre>
     *
     * @param wait A boolean indicating whether to block the thread until
     * timelapse is complete or not.
     * @param pictureName A String containing name for each captured image.
     * @param time Period of time in milliseconds to wait between taking each
     * image.
     * @return If the link latest image property is set, the File images are
     * being linked to, otherwise returns null.
     * @throws IOException
     * @throws InterruptedException
     */
...
///////////////////////////////////////////////////////////////////////////////
    ...
    /**
     * Gets the raspistill command previously executed by the RPiCamera. If no
     * commands have been executed, null will be returned.
     *
     ...
     *
     * @param saveDir String containing directory for RPiCamera to save images
     * to.
     */
...
//	}
    /**
     * Sets all RPiCamera options to their default settings, overriding any
...
///////////////////////////////////////////////////////////////////////////////
    ...
    /**
     * Turns on image preview. Note the preview will be superimposed over the
     * top of any other windows/graphics.
     */
    ...
    /**
     * Runs the preview windows using the full resolution capture mode. Maximum
     * frames per second in this mode is 15fps and the preview will have the
     * same field of view as the capture. Captures should happen more quickly as
     * no mode change should be required. This feature is currently under
     * development.
     */
    ...
    /**
     * Defines the size and location on the screen that the preview window will
     * be placed. Note the preview will be superimposed over the top of any
...
///////////////////////////////////////////////////////////////////////////////
    /**
     * Sets sharpness of image (-100 to 100), default value is 0. Sharpness
     * values lower than -100 will set sharpness to -100, values greater than
     * 100 will set sharpness to 100.
     *
    ...
    /**
     * Sets contrast of image (-100 to 100), default value is 0. Contrast values
     * lower than -100 will set contrast to -100, values greater than 100 will
     * set contrast to 100.
     *
    ...
    /**
     * Sets brightness of image (0 to 100), default value is 50. 0 is black, 100
     * is white. Brightness values lower than 0 will set brightness to 0, values
     * greater than 100 will set brightness to 100.
     *
    ...
    /**
     * Sets colour saturation of image (-100 to 100), default is 0. Saturation
     * values lower than -100 will set saturation to -100, values greater than
     * 100 will set saturation to 100.
     *
    ...
    /**
     * WARNING: OPERATION NOT YET SUPPORTED BY raspistill SOFTWARE. OPTION MAY
     * STILL BE SET, BUT WILL HAVE NO EFFECT ON THE IMAGE UNTIL SUPPORT IS ADDED
     * TO raspistill.<br>
     * <br>
...
//	MAKE METHOD FOR EVCOMPENSATION (-25 - 25?), ONLY FOR VIDEO
    /**
     * Sets exposure mode of RPiCamera. Certain modes may not be supported by
     * raspistill software, depending on camera tuning.
     *
    ...
    /**
     * Sets Automatic White Balance (AWB) mode. Certain modes may not be
     * supported by raspistill software, depending on camera type.
     *
    ...
    /**
     * Sets colour effect of RPiCamera. The specified U and V parameters (0 to
     * 255) are applied to the U and Y channels of the image. For example,
     * setColourEffect(128, 128) should result in a monochrome image.
     *
     * @param U
     * @param V
     */
    ...
    /**
     * Allows the specification of the area of the sensor to be used as the
     * source for the preview and capture. This is defined as x,y for the top
     * left corner, and a width and height, all values in normalised coordinates
     * (0.0-1.0). So to set a ROI at half way across and down the sensor, and a
     * width and height of a quarter of the sensor use :<br>
     ...
     * currently an upper limit of approximately 6000000us (6000ms, 6s). Shutter
     * speed values passed in that are greater than 6000000 will be regarded as
     * 6000000.
     *
     * @param speed
     */
...
///////////////////////////////////////////////////////////////////////////////	
    /**
     * Sets width of images taken by RPiCamera. Note that this setting can be
     * overriden by the takeStill() and takeBufferedStill() methods.
     *
    ...
    /**
     * Sets height of images taken by the RPiCamera. Note that this settings can
     * be overriden by the takeStill() and takeBufferedStill() methods.
     *
     ...
     * Quality values lower than 0 will set quality to 0, values greater than
     * 100 will set quality to 100.
     *
     ...
     * be easily accessed while the lapse is still running at the specified
     * file, while still saving all photos with their unique file names.
     *
     ...
     * milliseconds before taking the image. Default value is 5 seconds. If
     * timeout value is 0, then RPiCamera will continue to run indefinitely (but
     * can be stopped by calling stop() on the instance).
     *
    ...
    /**
     * Allows specification of the thumbnail image inserted in to the image
     * file. If not specified, defaults are a size of 64x48 at quality 35.
     *
     * @param x
     * @param y
     * @param quality
    ...
    /**
     * Sets the encoding for images. Default is JPG. Note that unaccelerated
     * image types (gif, png, bmp) will take much longer to save than JPG which
     * is hardware accelerated. Also note that the filename suffix is completely
    ...
    /**
     * Selects which camera to use (on a multicamera system). 0 or 1. Default is
     * 0. Values lower than 0 will select camera 0, values higher than 1 will
     * select camera 1.
     *
     * @param camNumber
     */
